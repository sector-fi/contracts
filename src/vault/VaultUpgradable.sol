...
contract VaultUpgradable is Initializable, ERC20, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	function cleanWithdrawalQueue() public {
		...
		for (uint256 i = 0; i < dirtyQueue.length; i++) {
			...
			if (!getStrategyData[strategy].trusted || isDuplicate(strategy)) continue;
			withdrawalQueue.push(strategy);
		}
